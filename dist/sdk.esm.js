...
var WBNB = (_WBNB = {}, _WBNB[ChainId.MAINNET] = /*#__PURE__*/new Token(ChainId.MAINNET, '0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c', 18, 'WBNB', 'Wrapped BNB'), _WBNB);
...
var CurrencyAmount = /*#__PURE__*/function (_Fraction) {
  ...
  CurrencyAmount.bnb = function bnb(amount) {
...
var Price = /*#__PURE__*/function (_Fraction) {
  ...
  _proto.quote = function quote(currencyAmount) {
    ...
    return CurrencyAmount.bnb(_Fraction.prototype.multiply.call(this, currencyAmount.raw).quotient);
  };
...
var Pair = /*#__PURE__*/function () {
  function Pair(tokenAmountA, tokenAmountB, factoryAddress, initCodeHash, lpTokenSymbol, lpTokenName) {
    ...
    this.liquidityToken = new Token(tokenAmounts[0].token.chainId, Pair.getAddress(tokenAmounts[0].token, tokenAmounts[1].token, this.factoryAddress, this.initCodeHash), 18, this.lpTokenSymbol, this.lpTokenName);
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    return [outputAmount, new Pair(inputReserve.add(inputAmount), outputReserve.subtract(outputAmount), this.factoryAddress, this.initCodeHash, this.lpTokenSymbol, this.lpTokenName)];
  };
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    return [inputAmount, new Pair(inputReserve.add(inputAmount), outputReserve.subtract(outputAmount), this.factoryAddress, this.initCodeHash, this.lpTokenSymbol, this.lpTokenName)];
  };
...
var Route = /*#__PURE__*/function () {
  function Route(pairs, input, output) {
    ...
    !(input instanceof Token && pairs[0].involvesToken(input) || input === BNB && pairs[0].involvesToken(WBNB[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'INPUT') : invariant(false) : void 0;
    !(typeof output === 'undefined' || output instanceof Token && pairs[pairs.length - 1].involvesToken(output) || output === BNB && pairs[pairs.length - 1].involvesToken(WBNB[pairs[0].chainId])) ? process.env.NODE_ENV !== "production" ? invariant(false, 'OUTPUT') : invariant(false) : void 0;
 ...
 * In other words, if the currency is BNB, returns the WBNB token amount for the given chain. Otherwise, returns
 * the input currency amount.
 */

function wrappedAmount(currencyAmount, chainId) {
  if (currencyAmount instanceof TokenAmount) return currencyAmount;
  if (currencyAmount.currency === BNB) return new TokenAmount(WBNB[chainId], currencyAmount.raw);
   process.env.NODE_ENV !== "production" ? invariant(false, 'CURRENCY') : invariant(false) ;
}

function wrappedCurrency(currency, chainId) {
  if (currency instanceof Token) return currency;
  if (currency === BNB) return WBNB[chainId];
   process.env.NODE_ENV !== "production" ? invariant(false, 'CURRENCY') : invariant(false) ;
}
/**
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    this.inputAmount = tradeType === TradeType.EXACT_INPUT ? amount : route.input === BNB ? CurrencyAmount.bnb(amounts[0].raw) : amounts[0];
    this.outputAmount = tradeType === TradeType.EXACT_OUTPUT ? amount : route.output === BNB ? CurrencyAmount.bnb(amounts[amounts.length - 1].raw) : amounts[amounts.length - 1];
  ...
  _proto.minimumAmountOut = function minimumAmountOut(slippageTolerance) {
    ...
    if (this.tradeType === TradeType.EXACT_OUTPUT) {
      ...
      return this.outputAmount instanceof TokenAmount ? new TokenAmount(this.outputAmount.token, slippageAdjustedAmountOut) : CurrencyAmount.bnb(slippageAdjustedAmountOut);
    }
  }
  /**
  ...
  _proto.maximumAmountIn = function maximumAmountIn(slippageTolerance) {
    ...
    if (this.tradeType === TradeType.EXACT_INPUT) {
      ...
      return this.inputAmount instanceof TokenAmount ? new TokenAmount(this.inputAmount.token, slippageAdjustedAmountIn) : CurrencyAmount.bnb(slippageAdjustedAmountIn);
    }
  }
  /**
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    var bnbIn = trade.inputAmount.currency === BNB;
    var bnbOut = trade.outputAmount.currency === BNB; // the router does not support both bnb in and out

    !!(bnbIn && bnbOut) ? process.env.NODE_ENV !== "production" ? invariant(false, 'BNB_IN_OUT') : invariant(false) : void 0;
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider, factoryAddress, initCodeHash, lpTokenSymbol, lpTokenName) {
    try {
      ...
      return Promise.resolve(new Contract(address, IUniswapV2Pair.abi, provider).getReserves()).then(function (_ref) {
        ...
        return new Pair(new TokenAmount(tokenA, balances[0]), new TokenAmount(tokenB, balances[1]), factoryAddress, initCodeHash, lpTokenSymbol, lpTokenName);
      });
    } catch (e) {
      return Promise.reject(e);
    }
